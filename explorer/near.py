from explorer.prediction import pred
...
from explorer.prompts import format_stats_for_prompt, generate_ai_response,generate_anomaly_analytics_prompt,generate_ai_response_anomaly
...
def display_basic_data(df):
    st.subheader("Basic FXS-USD analysis")
...
def beta_calculation(df):
    st.subheader("Market Sensitivity Analysis: FXS-USD vs. BTC-USD")
    ...
    aligned_data = pd.merge(market_ret.rename('Market'), near_ret.rename('FXS'), left_index=True, right_index=True, how='inner')

    # Perform linear regression
    slope, intercept, r_value, p_value, std_err = linregress(aligned_data['Market'], aligned_data['FXS'])
    ...
    plt.subplots()
    plt.scatter(aligned_data['Market'], aligned_data['FXS'], alpha=0.5)
    plt.plot(aligned_data['Market'], intercept + slope * aligned_data['Market'], 'r', label='fitted line')
    plt.xlabel('BTC-USD Returns')
    plt.ylabel('FXS-USD Returns')
    plt.title('Market Sensitivity Analysis: FXS-USD vs. BTC-USD')
    plt.legend()
    st.pyplot(plt)
    plt.close()
...
def generate_input_prompt(anomaly_summary):
    prompt = "Here is a list of detected whales/sharks in particular months:\n\n"
    for month_year, count in anomaly_summary.items():
        prompt += f"{month_year}: {count} \n\n"
        st.info(f"{month_year}: {count} \n\n Whales/sharks/anomalies")
...
def anomaly_detection(df):
    ...
    if not anomaly_data.empty:
        ...
        #analytics_response = generate_ai_response_anomaly(analytics_prompt, "key")
